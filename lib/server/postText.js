...
// The reading page renders the cover ABOVE the title, so a body that opens
// with the same image shows it twice, back to back.
//
// This happened over MCP because upload_blog_image used to hand back a ready
// made `![](url)` alongside `cover: true` — two contradictory instructions in
// one payload — and a model reasonably followed both. The tool no longer does
// that; this is the safety net for anything already written that way.
//
// Only a LEADING duplicate is removed. An image deliberately reused later in
// an article is the author's choice and is left alone.
export function stripLeadingCover(body, coverUrl) {
  const cover = String(coverUrl || "").trim();
  const src = String(body || "");
  if (!cover) return { body: src, removed: false };

  const lead = src.replace(/^\s+/, "");
  // ![alt](url), allowing the optional "title" some editors add, and the
  // <angle-bracket> form.
  const m = /^!\[[^\]]*\]\(\s*([^\s)]+)(?:\s+"[^"]*")?\s*\)[^\S\r\n]*(?:\r?\n)?/.exec(lead);
  if (!m) return { body: src, removed: false };

  const found = m[1].replace(/^<|>$/g, "");
  if (found !== cover) return { body: src, removed: false };

  return { body: lead.slice(m[0].length).replace(/^\s+/, ""), removed: true };
}
