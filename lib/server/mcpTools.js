...
export const TOOLS = [
  ...
  },
  {
    ...
    handler: async (args, { idToken }) => {
      ...
      const doc = {
        ...
        excerpt: args.excerpt || excerptFrom(args.body),
        cover: args.cover || "",
        tags: Array.isArray(args.tags) ? args.tags : [],
        readingTime: readingMinutes(args.body),
        published,
  ...
  },
  {
    ...
    handler: async (args, { idToken }) => {
      ...
      if (patch.body) patch.readingTime = readingMinutes(patch.body);
      if (cur.source === "devto") patch.editedHere = true;
      ...
      return { url: `/blog/${args.slug}`, changed: Object.keys(patch), rollbackVersion: version };
    },
  },
  {
    name: "delete_post",
  ...
  /* ---------- gallery ---------- */
  {
    ...
    description:
      "Add a photo to the site's photo wall. Send the image base64-encoded; it goes to the same storage as blog images and appears on the site straight away.",
    scope: "write",
    inputSchema: {
      type: "object",
      required: ["filename", "contentBase64"],
