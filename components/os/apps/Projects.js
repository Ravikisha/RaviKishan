...
// the "reasoning" reveal modal) — reflowed for a window with a sticky toolbar.
// In recruiter mode the routed /projects page is used instead.
const FILTERS = [
  { key: "all", label: "All" },
...
function simScore(p, terms) {
  ...
  if (raw === 0) return 0;
  const jitter = (hash(p.name + terms.join()) % 40) / 1000;
    ...
    <div className="flex flex-1 flex-col p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1" onClick={(e) => e.stopPropagation()}>
          ...
          {p.docs && (
            <a href={p.docs} target="_blank" rel="noopener noreferrer" aria-label={`${p.name} documentation`}
              title="Documentation site" className="rounded-md p-1.5 text-muted transition-colors hover:text-accentText">
              <BookOpen className="h-4 w-4" />
            </a>
          )}
          {p.link && (
            <a href={p.link} target="_blank" rel="noopener noreferrer" aria-label={`Open ${p.name}`}
              className="rounded-md p-1.5 text-muted transition-colors hover:text-fg">
              {isNpm(p) ? <Package className="h-4 w-4" /> : <ExternalLink className="h-4 w-4" />}
            </a>
          )}
        </div>
      </div>
...
const portOf = (p) => ((p.tags || []).includes("live") || p.link || p.docs ? "8080:8080" : "—");
...
const ThinkingModal = ({ project: p, onClose }) => {
  ...
  return (
    // z above even a fullscreen OS window (Z_FULL/Z_LAUNCH are 200000/300000)
    <div
      className="fixed inset-0 grid place-items-center bg-black/60 p-5 backdrop-blur-sm"
      style={{ zIndex: 300002 }}
      onMouseDown={onClose}
    >
      <motion.div
...
export default function Projects() {
  ...
  const list = searchMode ? scored || [] : browse.map((p) => ({ p, sim: null }));
    ...
    <div className="min-h-full bg-bg font-sans text-fg antialiased">
      ...
      <div className="sticky top-0 z-10 border-b border-edge bg-surface/95 px-5 py-3 backdrop-blur">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-accentText">Projects</p>
          <p className="font-mono text-[11px] text-muted">
            slice of <span className="text-accentText">{github?.repos || 69}</span> repos
          </p>
        </div>

        {/* vector search */}
        <div className="mt-3 flex items-center gap-2 rounded-lg border border-edge bg-bg px-3 py-2 font-mono text-sm transition-colors focus-within:border-amber/50">
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 font-mono text-[11px] transition-colors ${
                  on ? "border-accent bg-accent text-accentFg" : "border-edge bg-surface text-muted hover:text-fg"
                }`}
              >
            ...
            FILTERS.map((f) => (
              <button
                ...
                className={`rounded-full border px-3 py-1 font-mono text-[11px] transition-colors ${
                  filter === f.key ? "border-accent bg-accent text-accentFg" : "border-edge bg-surface text-muted hover:text-fg"
                }`}
              >
                {f.label}
              </button>
      ...
      <div className="px-5 py-6">
        {searchMode && searching ? (
          <div className="mx-auto max-w-xl rounded-xl border border-edge bg-surface p-6 font-mono text-sm">
            <div className="text-muted">
              <span className="text-accentText">~</span> encoding “{query}” → 1536-d vector
            </div>
            <div className="mt-2 text-muted">› scanning {ordered.length} project embeddings…</div>
          ...
          <>
            {searchMode && list.length > 0 && (
              <div className="mb-6 font-mono text-xs text-muted">
                <span className="text-live">{list.length}</span> vectors matched · cosine ≥{" "}
                <span className="text-accentText">{list[list.length - 1].sim.toFixed(2)}</span> · sorted by similarity
              </div>
            )}
      ...
      {selected && <ThinkingModal project={selected} onClose={() => setSelected(null)} />}
    </div>
  );
}
