...
// GitHub — LIVE. scripts/sync-metrics.mjs refreshes lib/metrics.json on a
// nightly GitHub Actions cron and commits it; Vercel redeploys on the push.
// Do not hand-edit these numbers: the next sync overwrites them. The literals
// below are only a fallback if metrics.json is ever missing or malformed.
//
// `repos` counts ORIGINALS ONLY (forks excluded) — the honest reading of
// "repos I built". metrics.github.reposIncludingForks has the other number.
export const github = {
  stars: metrics?.github?.stars ?? 167,
  repos: metrics?.github?.repos ?? 69,
  followers: metrics?.github?.followers ?? 39,
  since: metrics?.github?.since ?? 2019,
};

// npm downloads across every published package, last 12 months, rounded DOWN
// to a clean threshold so the public claim is never an overstatement.
export const npmDownloads = {
  total: metrics?.npm?.total ?? 0,
  claim: metrics?.npm?.claim ?? 1000,
  packages: metrics?.npm?.packages ?? 0,
};

export const metricsUpdated = metrics?.generatedAt || null;
...
// Per-route SEO overrides, editable from /admin. Empty strings mean "use the
// default built by components/Seo.js". Keys are pathnames, no trailing slash.
export const seo = {
  "/": { title: "", description: "", image: "" },
  "/about": { title: "", description: "", image: "" },
  "/projects": { title: "", description: "", image: "" },
  "/skills": { title: "", description: "", image: "" },
  "/resume": { title: "", description: "", image: "" },
...
export const buildLog = [
  ...
  {
    cmd: "downloads --pkg relax*",
    out: `${npmDownloads.claim.toLocaleString("en-US")}+ NPM downloads across the Relax ecosystem`,
    ok: true,
  },
...
export const stats = [
  ...
  { value: 1200, suffix: "+", label: "DSA problems solved" },
  {
    value: github.stars,
    suffix: "★",
    label: `GitHub stars across ${github.repos} repos`,
  },
];
...
export const chips = [
  "Patent holder",
  `${npmDownloads.claim.toLocaleString("en-US")}+ npm downloads`,
  "Gold Medalist",
];
...
export const systems = [
  {
    ...
    metric: `35% faster than React · ${npmDownloads.claim.toLocaleString("en-US")}+ npm downloads`,
