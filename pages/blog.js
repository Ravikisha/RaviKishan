...
// seed (lib/blogPosts.js) normalized to the page's shape — last-resort fallback
// if both the API route and the live dev.to fetch are unavailable.
...
const Featured = ({ p }) => (
  <motion.div
    ...
    className="group mb-8 grid cursor-pointer grid-cols-1 overflow-hidden rounded-2xl border border-edge bg-surface transition-colors hover:border-amber/40 lg:grid-cols-[1.1fr_1fr]"
  >
    <div className="relative aspect-[16/10] overflow-hidden border-b border-edge bg-bg lg:border-b-0 lg:border-r">
    ...
    <div className="flex flex-col justify-center p-7">
      ...
      <h2 className="mt-2 font-display text-2xl font-bold leading-tight text-fg group-hover:text-accentText sm:text-3xl">
        {p.title}
      </h2>
      {p.description && (
...
const PostCard = ({ p, i }) => (
  ...
  >
    ...
    <div className="flex flex-1 flex-col p-5">
      <p className="font-mono text-[11px] text-muted">{fmtDate(p.publishedAt)}</p>
      <h3 className="mt-2 font-display text-lg font-bold leading-snug text-fg group-hover:text-accentText">{p.title}</h3>
      {p.description && (
          ...
          {status === "ok" && posts.length > 0 && (
            <div className="space-y-4">
              <p className="font-mono text-xs text-muted">
                ...
                <span className="text-accentText">{crossposts}</span> also on Medium · sourced live
              </p>
            ...
            {status === "ok" && (
              <>
                ...
                <motion.div layout className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                  {rest.map((p, i) => <PostCard key={p.url || i} p={p} i={i} />)}
                </motion.div>
                {shown.length === 0 && (
