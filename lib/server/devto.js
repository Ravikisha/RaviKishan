...
// dev.to does not hand back the cover you uploaded. `cover_image` and
// `social_image` are its CDN's own derivatives:
//
//   https://media2.dev.to/dynamic/image/width=1000,height=420,fit=cover,
//     gravity=auto,format=auto/<the original, URL-encoded>
//
// which is a re-encoded WebP, cropped to a 2.4:1 letterbox and capped at
// 1000px wide — measured against one real article, 339 KB of WebP standing in
// for a 1024x1024, 1.95 MB PNG. This page renders a cover up to 1180 CSS px,
// so that thumbnail is being upscaled, and on a 2x display it is asked for
// 2360 device pixels. It looks soft because it IS soft.
//
// The original is sitting right there inside the URL. Take it.
export function originalImageUrl(url) {
  const u = String(url || "");
  const m = /^https?:\/\/[^/]*\bdev\.to\/dynamic\/image\/[^/]+\/(.+)$/.exec(u);
  if (!m) return u;
  try {
    const inner = decodeURIComponent(m[1]);
    // Only follow it if it really is an absolute URL; anything else means the
    // CDN changed its shape and the wrapper is safer than a guess.
    return /^https?:\/\//.test(inner) ? inner : u;
  } catch (_) {
    return u;
  }
}
...
export function toPost(a) {
  ...
  return {
    ...
    cover: originalImageUrl(a.cover_image || a.social_image || ""),
    tags: a.tag_list || [],
